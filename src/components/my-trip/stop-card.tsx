...
export function StopCard({
    ...
    return (
        <article
            className={cn(
                "relative rounded-3xl border border-line-subtle bg-surface-raised p-4 pl-14 transition-shadow",
                className,
            )}
            style={style}
        >
            <div className="absolute inset-y-0 left-0 w-12 rounded-l-3xl border-r border-line-subtle bg-brand-50/80" />
            <Button
                type="button"
                variant="ghost"
                size="icon"
                className="absolute left-1 top-1/2 z-10 h-[calc(100%-0.75rem)] w-9 -translate-y-1/2 rounded-2xl cursor-grab text-ink-body hover:bg-brand-100 hover:text-ink-strong active:cursor-grabbing"
